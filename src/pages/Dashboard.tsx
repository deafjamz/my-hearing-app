...
export function Dashboard() {
  ...
  const { history, dailyGoal, setDailyGoal } = useUser();

  const [showGoalModal, setShowGoalModal] = useState(false);
  const [tempGoal, setTempGoal] = useState(dailyGoal.toString());

  // Derive today's minutes from history
  const todayMinutes = useMemo(() => {
    const todayStr = new Date().toISOString().split('T')[0];
    return history.find(h => h.date === todayStr)?.minutes || 0;
  }, [history]);
                ...
                {viewMode === 'today' ? (
                  ...
                  <motion.div key="week" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="w-full h-full flex flex-col">
                    <div className="flex items-center justify-between w-full px-2 mb-6">
                      ...
                      <button onClick={() => setShowGoalModal(true)} className="text-slate-400 dark:text-slate-500 text-xs font-bold uppercase tracking-widest bg-purple-100 dark:bg-purple-900/30 px-2 py-1 rounded-md hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors">GOAL: {dailyGoal || 25} MIN</button>
                    </div>
                    <div className="flex-1 w-full min-h-[200px]"><ProgressHistory data={weekData} goal={dailyGoal} /></div>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 w-[90%] max-w-sm border border-slate-200 dark:border-slate-800 shadow-xl">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => { const newGoal = Number(tempGoal); if (!isNaN(newGoal) && newGoal > 0) { setDailyGoal(newGoal); } setShowGoalModal(false); }} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Save</button>
            </div>
          </div>
        </div>
      )}
