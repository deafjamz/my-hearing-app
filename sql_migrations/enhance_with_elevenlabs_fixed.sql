-- Enhancement Migration: Add ElevenLabs Metadata to Existing Schema (FIXED)
-- This ADDS features without destroying existing data
-- Safe to run - uses "ADD COLUMN IF NOT EXISTS"

-- =============================================================================
-- 1. ADD MISSING CLINICAL COLUMNS to word_pairs first
-- =============================================================================
...
ALTER TABLE word_pairs
  ...
  ADD COLUMN IF NOT EXISTS vowel_context TEXT,
  ADD COLUMN IF NOT EXISTS clinical_category TEXT;

-- =============================================================================
-- 2. ENHANCE WORD_PAIRS with ElevenLabs Metadata
-- =============================================================================
...
-- =============================================================================
-- 3. ENHANCE STORIES with ElevenLabs Metadata
-- =============================================================================
...
-- =============================================================================
-- 4. ENHANCE SENTENCES with ElevenLabs Metadata
-- =============================================================================
...
-- =============================================================================
-- 5. CREATE NOISE_ASSETS Table (New)
-- =============================================================================
...
-- =============================================================================
-- 6. POPULATE DEFAULT VOICE METADATA (From Clinical Analysis)
-- =============================================================================
...
-- =============================================================================
-- 7. POPULATE target_phoneme from category (best effort)
-- =============================================================================

UPDATE word_pairs
SET target_phoneme = CASE
  WHEN category ILIKE '%vowel%' THEN 'vowel'
  WHEN category ILIKE '%voicing%' THEN 'voicing'
  WHEN category ILIKE '%fricative%' THEN 'fricative'
  WHEN category ILIKE '%nasal%' THEN 'nasal'
  WHEN category ILIKE '%stop%' THEN 'stop'
  ELSE LOWER(REPLACE(category, ' ', '_'))
END
WHERE target_phoneme IS NULL AND category IS NOT NULL;
...
SELECT
  ...
  CASE
    ...
    WHEN f0_mean_hz_david IS NULL THEN 'Unknown'
    ELSE 'Suboptimal'
  END as david_ci_suitability,
  CASE
    WHEN f0_mean_hz_sarah BETWEEN 165 AND 265 THEN 'Optimal'
    WHEN f0_mean_hz_sarah IS NULL THEN 'Unknown'
    ELSE 'Suboptimal'
...
-- Show enhanced word_pairs structure (ElevenLabs columns)
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'word_pairs'
AND (column_name LIKE '%f0%' OR column_name LIKE '%rms%' OR column_name LIKE '%stoi%' OR column_name LIKE '%elevenlabs%')
...
SELECT id, word_1, word_2, category, target_phoneme, f0_mean_hz_david, f0_mean_hz_sarah
FROM word_pairs
LIMIT 3;
